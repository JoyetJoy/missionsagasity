...
router.put('/:id', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const existing = await prisma.prayer.findUnique({ where: { id: req.params.id } });
    if (!existing) return res.status(404).json({ error: 'Gathering not found' });
    ...
    if (isRecurring !== undefined) updateData.recurring_pattern = isRecurring ? recurringPattern : null;
